...
function PersonalData({ data }: MinimalTemplateProps) {
  ...
  return (
    <div className="bg-linear-to-r from-blue-900 to-blue-800 px-8 py-8 text-white">
      <h1 className="mb-2 text-3xl font-bold flex flex-col">
        {data.personalInfo.firstName} {data.personalInfo.lastName}
        <span className="text-base">{data.personalInfo.headline}</span>
      </h1>
      <div className="mt-4 flex flex-wrap gap-x-3 gap-y-2 text-sm">
        ...
        {(data.personalInfo.links ?? []).map((link) => (
          <a
            ...
            href={link.url}
            target="_blank"
            className="capitalize flex items-center gap-1 text-white underline"
          >
